...
def delInitials(L):
    ...
    """
    return ', '.join(x.capitalize() for x in L if len(x)>1 )
...
def everyOccurrence(S, Q):
    ...
    """
    #checkcaracter = lambda vS: reduce(lambda acc,vQ: acc or vQ==vS ,Q,False)
    return [ i for i,vS in enumerate(S) if (vS in Q)]
    #return [ i for i,vS in enumerate(S) if checkcaracter(vS)]
